...
colnames(alter_keeps)[1] <- "study_id"
...
############################ Older Info Finder ################################
#Older function that only works for names, keeping it around just in case.
# name_finder <- function(alter_set){
...
#     alter_names[i] <- unlist(workbench[paste("name",alter_set$alter_nums[i],sep = "")])
#   }
#   
#   return(alter_names)
# }

############################ Older Checkbox Finder ################################
# supp_type_finder <- function(alter_set){
#   if(length(unique(alter_set$study_id)) != 1){
#     stop("Too many study ID's in input, isolate them")
#   }
#   
...
#   workbench <- sample_data[sample_data$study_id == study_id,]
#   
#   #Assigning a set of 6 variables blank vectors per each category
#   supp_type1 <- supp_type2 <- supp_type3 <- c()
#   supp_type4 <- supp_type5 <- supp_type6 <- c()
#   
#   for(i in 1:length(alter_set$alter_nums)){
#     #Creating strings which will access the support type at the sepecified alter
#     supp_first <- sub("X",alter_set$alter_nums[i],"nameXsupptype___1")
#     supp_last <- sub("X",alter_set$alter_nums[i],"nameXsupptype___0")
#     
...
#     names(supp_set) <- c("Emotional support", "Life advice", "Financial support",
...
#     add_this <- supp_set[as.logical(supp_set)][1:6]
...
#     supp_type1[i] <- add_this[1]
#     supp_type2[i] <- add_this[2]
#     supp_type3[i] <- add_this[3]
#     supp_type4[i] <- add_this[4]
#     supp_type5[i] <- add_this[5]
#     supp_type6[i] <- add_this[6]
#   }
#   #Assigning everything to a list for export
#   supp_type <- list(supp_type1 = supp_type1, supp_type2 = supp_type2,
#                     supp_type3 = supp_type3, supp_type4 = supp_type4,
#                     supp_type5 = supp_type5, supp_type6 = supp_type6)
#   
#   return(supp_type)
# }
# 
# 
# supp_type <- by(alter_frame, alter_frame$study_id, supp_type_finder)
# 
# supp_type1 <- supp_type2 <- supp_type3 <- c()
# supp_type4 <- supp_type5 <- supp_type6 <- c()
# 
# for(i in 1:length(supp_type)){
#   supp_type1 <- append(supp_type1,supp_type[[i]][[1]])
#   supp_type2 <- append(supp_type2,supp_type[[i]][[2]])
#   supp_type3 <- append(supp_type3,supp_type[[i]][[3]])
#   supp_type4 <- append(supp_type4,supp_type[[i]][[4]])
#   supp_type5 <- append(supp_type5,supp_type[[i]][[5]])
#   supp_type6 <- append(supp_type6,supp_type[[i]][[6]])
# }
# 
# alter_frame$supp_type1 <- supp_type1
# alter_frame$supp_type2 <- supp_type2
# alter_frame$supp_type3 <- supp_type3
# alter_frame$supp_type4 <- supp_type4
# alter_frame$supp_type5 <- supp_type5
# alter_frame$supp_type6 <- supp_type6
...
alter_frame$sex <- factor(alter_frame$sex, levels = c(1,0))
...
alter_frame$fb <- unlist(by(alter_frame, alter_frame$study_id,
                            info_finder, suffix = "fb"))

alter_frame$fb <- factor(alter_frame$fb, levels = c(1, 2, 3, 0))
levels(alter_frame$fb) <- c("High school football", "College football",
                            "Professional football", "Not a football player")

############################# Adding fbwith ###################################

varpre_fbwith <- "nameXfbwith___1"
varpost_fbwith <- "nameXfbwith___0"
checkbox_fbwith <- c("Former Professional Teammate",
                     "Former Professional Coach",
                     "Former HS or College Teammate",
                     "Former HS or College Coach",
                     "Other Football Connection",
                     "No football connection")

list_fbwith <- checkbox_arranger(alter_frame, varpre_fbwith, varpost_fbwith, checkbox_fbwith)

for(i in 1:length(checkbox_fbwith)){
  alter_frame[paste("fbwith",i,sep = "")] <- factor(list_fbwith[[i]],
                                                   levels = checkbox_fbwith)
}
...
alter_frame$supplement <- unlist(by(alter_frame, alter_frame$study_id,
                                    info_finder, suffix = "supp"))

alter_frame$supplement <- factor(alter_frame$supplement, levels = c(1,0,9,99))
levels(alter_frame$supplement) <- c("Yes",
                                    "No", "Does not take PDE's or supplements",
                                    "Don't know")
...
levels(alter_frame$dist) <- c("Same house", "0-5 miles", "6-15 miles",
                              "16-15 miles", "50+ miles")
